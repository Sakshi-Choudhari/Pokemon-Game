...
level = 30
pikachu = Pokemon('Pikachu', level, 25, 150)
charmander = Pokemon('Charmander', level, 175, 150)
bulbasaur = Pokemon('Bulbasaur', level, 325, 150)
...
while game_status != 'quit':
    ...
    if game_status == 'prebattle':
        ...
        # Set moves for both player and rival Pokémon
    ...
    if game_status == 'rival turn':
        ...
        # Check if rival_pokemon has moves available
        if rival_pokemon.moves:
            ...
            display_message(f"{rival_pokemon.name} has no available moves!")
            time.sleep(2)
            game_status = 'player turn'
